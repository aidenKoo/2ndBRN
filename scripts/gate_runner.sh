...
run_behavioral_gate() {
  ...
  bash scripts/harness.sh evolve >/dev/null
  bash scripts/harness.sh heal >/dev/null
  pass "harness check/packet/report/evolve/heal"
}

run_tool_gate() {
  echo "== tool-gate =="
  if python3 scripts/project_tool_gates.py; then
    pass "project tool gates completed"
  else
    fail "project tool gates failed"
  fi
}
...
run_documentation_gate() {
  ...
  if [[ -f reports/self-heal-report.md ]]; then
    pass "self-heal report exists"
  else
    warn "self-heal report missing (run: bash scripts/harness.sh heal)"
  fi
}
